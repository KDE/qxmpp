...
/*!
    ...
    \brief History options for joining a \xep{0045}{Multi-User Chat} room (§7.2.13).
...
/*!
    \enum QXmpp::Muc::LeaveReason
    \inmodule QXmpp

    \brief Reason why a participant left a \xep{0045}{Multi-User Chat} room.

    \since QXmpp 1.16

    \value Left The participant left the room voluntarily.
    \value Kicked The participant was kicked by a moderator (§8.2, status 307).
    \value Banned The participant was banned from the room (§8.4, status 301).
    \value AffiliationChanged The participant was removed because their affiliation changed (status 321).
    \value MembersOnly The participant was removed because the room became members-only (status 322).
    \value RoomDestroyed The room was destroyed by its owner (§10.9).
    \value ServiceShutdown The participant was removed because the MUC service is being shut down (status 332).
    \value Error The participant was removed because of an error (status 333).
    \value ConnectionLost The client detected via \xep{0410}{MUC Self-Ping} that it had been silently removed.
*/
...
/*!
    \class QXmpp::Muc::Destroy
    \inmodule QXmpp

    \brief Information about a destroyed \xep{0045}{Multi-User Chat} room (§10.9).
...
/*!
    ...
    \brief A \xep{0045}{Multi-User Chat} room item carrying role and/or affiliation data.
...
/*!
    \struct QXmpp::Muc::Avatar
    \inmodule QXmpp

    \brief Room avatar data containing a MIME content type and raw image bytes.
...
/*!
    \class QXmpp::Muc::DirectInvitation
    \inmodule QXmpp

    \brief A direct \xep{0045}{Multi-User Chat} invitation as defined by
    ...
    to a group chat (\xep{0045}{Multi-User Chat} §7.9).
...
/*!
    \class QXmpp::Muc::Invite
    \inmodule QXmpp

    \brief A mediated \xep{0045}{Multi-User Chat} invitation (§7.8.2).
...
/*!
    \class QXmpp::Muc::Decline
    \inmodule QXmpp

    \brief A mediated \xep{0045}{Multi-User Chat} invitation decline (§7.8.2).
...
/*!
    \class QXmpp::Muc::UserQuery
    \inmodule QXmpp

    \brief The \c x element with namespace \c muc#user as defined by \xep{0045}{Multi-User Chat}.
