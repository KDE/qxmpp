...
template<typename T>
bool isIqType(const QDomElement &el) { return Private::isPayloadType<T>(Private::iqPayloadXmlTag(el)); }

/// \brief Checks whether an XML element has the correct tag name and namespace URI for type T.
template<Private::HasXmlTag T>
bool isElementType(const QDomElement &el) { return Private::elementXmlTag(el) == T::XmlTag; }
